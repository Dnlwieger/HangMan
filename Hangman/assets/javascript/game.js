...
// blank array for used letters

var usedLetters = [];
var wordChoice = "";
var getWord = function () { 
	 wordChoice = pokemon[Math.floor(Math.random()*pokemon.length)].toLowerCase();
};
console.log(pokemon);
var blanks = [];
var blankImage = "";

// create empty "_" spaces based on the chosen word
var makeBlanks = function(){
for(var i = 0; i < wordChoice.length; i++) {
  blanks.push("_&nbsp;");
  // blanks.toString();
  blankImage = blanks.join("");
  document.getElementById("word").innerHTML = blankImage;
};
};

// have userGuess replace these spaces appropriately

var checkMatch = function () {

        var includeLet = letters.includes(userGuess);
        var randomWordIn = wordChoice.indexOf(userGuess);
        var indexUl = usedLetters.indexOf(userGuess);
        if (includeLet === false) {
            console.log("not a valid guess");
        } else if (randomWordIn !== -1) {      
            for (i = 0; wordChoice.indexOf(userGuess, i) !== -1; i++) {
                blanks.splice(wordChoice.indexOf(userGuess, i), 1, userGuess);
                blankImage = blanks.join(" ");
                testWord = blanks.join (""); 
                document.getElementById("word").innerHTML = blankImage; 
                }
        } else if (randomWordIn === -1 && indexUl === -1) {
                usedLetters.push(userGuess);
                usedImage = usedLetters.join();
                lives--;
                document.getElementById("usedImageDisplay").innerHTML = usedImage;
                document.getElementById("livesDisplay").innerHTML = lives;
            }
}
// Check for win/lose
var checkWins = function (){
	if (wordChoice === testWord) { wins++;
...
// reset the game functions for a new game, but keep win count unless reload page
var  reset = function (){
     flag = true;
	 testWord = "";
	 usedImage = "";
...
// call appropriate functions & reload the DOM
...
//capture the user's keystrokes & change them to lowercase
